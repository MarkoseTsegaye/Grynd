...
- **`resumeWorkoutEntry(splitId)` store action:** clears `pausedAt` only for the matching split; no-ops when split differs or session is already unpaused. Called during workout screen bootstrap after a matching active session is found (before `setBootstrapState('ready')`).
