...
- **`hasPausedSession` / `shouldPromptResumeSession` helpers in `workoutRoute.ts`:** `hasPausedSession(session)` treats any incomplete session (`completedAt === null`) as paused regardless of `pausedAt` presence (backward compat with legacy sessions). `shouldPromptResumeSession(session, pathname)` combines `hasPausedSession` + `!isWorkoutRoute` for a single prompt-eligibility predicate. Both exported via `src/features/workout/index.ts`.
- **Home resume CTA consolidated into `PausedWorkoutResumeCard`:** replaced two conditional blocks (Today split match vs. other split) with a single shared component shown when `hasPausedSession(activeSession)` is true, independent of today's split. Today card always shows "Start Workout"; the resume card appears above it when a paused session exists.
...
- **Home conflict guard via `startWorkoutForSplit`:** All "Start Workout" taps on Home (Today card and All Splits list) are routed through `startWorkoutForSplit(targetSplitId)`. When `hasPausedSession(activeSession)` is true and the target split differs from the paused split, it shows an "Unfinished Workout" native alert (Discard → `abandonWorkout()` then push target; Resume → push paused split). Same-split taps proceed directly. Alert uses `router.push` (not `replace`) — appropriate since no active workout screen is in the stack.
